...
async function exec(config, postBody) {
  ...
  const currentFileName = postBody.currentFileName;  // コピー対象のファイル名
  const newFileName     = postBody.newFileName;      // コピー先のファイル名
  ...
  try {
    await fs.promises.copyFile(sourceFilePath, targetFilePath);  // 途中に存在しないディレクトリなどを書いていた場合は移動に失敗する
    console.log(`\n${JSON.stringify({ result: 'OK' })}`);
  }
  catch(_error) {
    responseError('Failed To Copy');
  }
}
