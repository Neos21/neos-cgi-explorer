...
async function exec(config, postBody) {
  ...
  const directoryName = postBody.directoryName;  // 作成するディレクトリ名 (スラッシュが含まれていると Recursive に作られる)
  if(!directoryPath || !directoryName) return responseError('Invalid Parameters');
  
  const targetDirectoryPath = path.resolve(config.rootDirectoryPath, directoryPath, directoryName);
  if(!targetDirectoryPath.startsWith(config.rootDirectoryPath)) return responseError('Invalid Path');  // ルートディレクトリ配下になっていない場合は中断する
  
  try {
    await fs.promises.stat(targetDirectoryPath);
    return responseError('The Directory Already Exists');
  }
  catch(_error) { }
